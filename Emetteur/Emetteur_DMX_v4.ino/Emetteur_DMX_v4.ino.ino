...
char modes[nbr_canaux] = {0, 0, 0, 0, 0, 0};
...
void loop () { // Boucle du programme principal
  ...
  for (int i = 0; i < nbr_canaux; i++) {          // Lecture des canaux et stockage des modes 
  ...
  for (int i = 2; i <= 5; i++) { // envoit des messages pour chaque récepteur 
    int index = (i-2)*2 ; 
    char radiopacket[3] ;
    radiopacket[0] = modes[index] ; 
    radiopacket[1] = modes[index+1] ; 
    
    // if (modes[i] != modes_memory[i]) {
    //digitalWrite(LedDMX, HIGH);
    //modes_memory[i] = modes[i] ;
    
    //int rang = i - (i / 2) * 2 ;
    //radiopacket[rang] = modes[i] ; // modes_memory[i] ;
    //RECEIVER = (i / 2) + 2 ;
    ...
    /*
      ...
      int rang = i - (i / 2) * 2 ;
      radiopacket[rang] = modes_memory[i] ;
      RECEIVER = (i / 2) + 2 ;
